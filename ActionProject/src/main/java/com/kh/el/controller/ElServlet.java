...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// JSO로 포워딩 할 때 request에 담아서 보내줬음! -- Servlet 내장객체 / scope객체
		 ...
		 * 이 영역에 데이터를담으면 JSP/Servlet단에서 사용가능
		 * 값이 한 번 담기면 서버를 멈추거나, 세션을 비우거나, 브라우저가 닫히기 전까지는 사용가능
		 * 
		 * 
		 ...
		 * KSP페이지 내에서만 사용 가능 
		 * 
		 * => 위 객체들에 속성을 추가할 때는	.serAttribute("키",밸류);
		 * 							.getAttribute("키");
		 * 							.removeAttribute("키");
		 * 
		 */
		...
		request.setAttribute("key", "session Key");
