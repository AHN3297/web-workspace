...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 3. 결과값 반환(응답화면 지정)
		 * 
		 *View에서 Get방식으로 요청시 doGet()가 호풀됨
		 * 
		 */
		 ...
		 * => 요청 전송방식. 요청 URL, 요청한 사용자의 정보, 사용자가 input요소에 입력한 값 등.
		 * 
		 * 두 번째 매개변수 HttpServletResponse타입은 요청 처리 후 응답할 때 사용하는 객체
		 * 
		 * 요청 처리 스텝
		 * 
		 ...
		 * 
		 * -request.getParameter("키값") : String(input요소에 적어놓은 name 속성값)
		 * => 반환형이String이기 때문에 다른 자료형으로 사용해야한다면 Parsing해야함
		 * 
		 * - request.getParameterValues("키값") : String[]
		 * => 하나의 key값으로 여러개의 value들을 받아야할 경우
		 */
		
		// 1단계. 값뽑기
		String name = request.getParameter("name");
		System.out.println(name);
		// 들어올 수 있는 키값의 수는 "셀 수 없음"
		...
		// 		=> 경로를 잘 못 적었거나 피일명에 오타가 났을 때
		// 500 : 자바코드상의 오류(예외발생)
		
		// 2단계. 데이터가공
		// Person person = newPerson(name,gender, age, city, height, foods);
		// 만들었다 침
		// 3단계. 요청 처리(DB와의 상호작용 ==JDBC/MyBatis)
		// 보통의 흐름 : Controller에서 Service의 메소드를호출하면서 값을 전달
		// -> DAO호출 -> DBSQL문(INSERT)문 실행 -> 정수형태의 결과값 반환
		// int result = new PersonService().save.Person(person);
