...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 
		 * -request : 서버로 요청 보낼 때의 정보(요청 시 전달값, 요청 전송 방식, 사용자 정보)
		 * -response : 요청에 대한 응답데이터를 만들 때 사용
		 */
		// 절차
		// 1) GET? POST? => 요청방식이 POST방식이라면 인코딩 작업
		request.setCharacterEncoding("UTF-8");
		
		// 2) 요청 시 전달값이 있나? 없나? => POST방식은 무조건 해야함
		...
		// 4) 응답화면 지정
		// 스템 1. request속성에 출력할 값 추가 => setAttribute()
		// 스텝 2. RequestDispatcher => 뷰지정
		// 스텝 3. RequestDispatcher이용해서 forward()호출
		
		// 1. 로그인 된 사용자의 정보를 어딘가에 담을것(application, session,
		// request, page)
		
		/*
		 * 크다
		 * 1) application : 웹 애플리케이션 전역에서 사용가능
		...
		// 로드인에 성공할 수 도 있음 . 실패할 수도 있음
		// 2. 성공실패여부에 따라서 응답 페이지 다르게 보내주기
